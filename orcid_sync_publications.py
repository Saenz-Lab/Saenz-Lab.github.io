...
- Deduplicates entries by DOI (keeps the one with the best metadata).
"""
...
def metadata_score(item: Dict[str, Any]) -> int:
    """Score an item by metadata completeness for dedup preference."""
    score = 0
    if item.get("journal"):
        score += 1
    if item.get("doi"):
        score += 1
    # Prefer DOI links over institutional repository links
    link = item.get("link", "")
...
def deduplicate(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Deduplicate by DOI, keeping the entry with the best metadata."""
    ...
    for item in items:
        ...
        doi_lower = doi.lower()
        if doi_lower not in seen_doi:
            seen_doi[doi_lower] = item
        else:
            # Keep the one with better metadata
            if metadata_score(item) > metadata_score(seen_doi[doi_lower]):
                seen_doi[doi_lower] = item
